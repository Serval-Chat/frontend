...
export const PollVotersModal: React.FC<PollVotersModalProps> = ({
    ...
    const { data: users, isLoading } = useUsers(allVoterIds);

    const selectedOption = poll.options?.find(
        (opt) => opt.id === selectedOptionId,
    );
    const voterIdsForSelected = selectedOption?.votes || [];

    const votersForSelected = useMemo(() => {
        if (!users) return [];
        return users.filter((u) => voterIdsForSelected.includes(u._id));
    }, [users, voterIdsForSelected]);
